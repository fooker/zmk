...
K_THREAD_STACK_DEFINE(service_q_stack, CONFIG_ZMK_SPLIT_SERIAL_WRITE_STACK_SIZE);

struct k_work_q service_work_q;

K_MSGQ_DEFINE(position_state_msgq, sizeof(char[SPLIT_DATA_LEN]),
              CONFIG_ZMK_SPLIT_SERIAL_QUEUE_SIZE, 4);
...
static int split_serial_service_init(const struct device *dev) {
    ...
    k_work_queue_start(&service_work_q, service_q_stack, K_THREAD_STACK_SIZEOF(service_q_stack),
                       CONFIG_ZMK_SPLIT_SERIAL_WRITE_RUN_PRIORITY, &queue_config);
